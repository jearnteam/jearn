import { TranslationSchema } from "./en";

const my: TranslationSchema = {
  translation: {
    loading: "ခဏစောင့်ပါ",
    loadingPosts: "ပို့စ်တွေကိုရှာနေပါတယ်",
    jearn: "JEARN",
    logout: "အကောင့်ထွက်မည်",
    login: "အကောင့်ဝင်မည်",
    notLoggedIn: "အကောင့်မဝင်ထားပါ",
    post: "ပို့စ်",
    question: "မေးခွန်း",
    questionEnter: "သင့်မေးခွန်းကို ထည့်သွင်းပါ။",
    answer: "Answer",
    title: "ခေါင်းစဥ်",
    placeholder: "အားလုံးနဲ့မျှဝေချင်တဲ့အကြောင်းအရာကိုရေးကြမယ်",
    cancel: "ပယ်ဖျက်မည်",
    share: "မျှဝေမည်",
    edit: "ပြုပြင်မည်",
    delete: "ဖျက်မည်",
    report: "တိုင်ကြားမည်",
    reported: "တိုင်ကြားပြီး",
    createPost: "ပို့စ်အသစ်ရေးမယ်",
    editPost: "ပို့စ်ကိုဘယ်လိုပြုပြင်ချင်ပါသလဲ?",
    saveChanges: "ပြောင်းလဲမှုကိုသိမ်းမည်",
    postingAsBefore: "",
    postingAsAfter: "အနေနဲ့ပို့စ်တင်မည်",
    editingAsBefore: "",
    editingAsAfter: "အနေနဲ့ပြုပြင်မည်",
    checkCategories: "ပို့စ်အမျိုးအစားကိုစစ်မည်",
    submit: "ပို့စ်တင်မည်",
    showMore: "ပိုကြည့်မည်",
    showLess: "ပြန်သိမ်းမည်",
    deletePost: "ပို့စ်ကိုဖျက်မည်",
    deletePostDesc: "ပို့စ်ဖျက်ပြီးရင်ပြန်မရနိုင်ပါ တကယ်ဖျက်မှာသေချာပါသလား",
    shareThisPost: "ပို့စ်ကို မျှဝေမည်",
    copied: "ကူးယူပြီး!",
    copyLink: "ပို့စ်လင့်ခ်ကိုကူးယူမည်",
    categoryName: "အမျိုးအစားအမည်",
    search: "ရှာဖွေမည်",
    categoryReason: "အကြောင်းပြချက် (Optional)",
    whyReason: "ဘာကြောင့် ဒီအမျိုးအစားကို လိုအပ်တာလဲ?",
    loadingUser: "Loading User",
    requestSent: "အောင်မြင်စွာတောင်းဆိုချက်ပေးပို့ပြီးပါပြီ!",
    sendRequest: "တောင်းဆိုချက်ပေးပို့မည်",
    answerToQuestion: "မေးခွန်းကို ဖြေဆိုမည်",
    postAnswer: "ဖြေဆိုမည်",
    answers: "အဖြေများ",
    noAnswersYet: "အဖြေ မရှိသေးပါ",

    // home page
    home: "ပင်မစာမျက်နှာ",
    follow: "Followလုပ်ထားသူများ",
    notifications: "အသိပေးချက်များ",
    videos: "ဗီဒီရို",
    chat: "Chat",

    //poll
    createPoll: "Create Poll",
    createPollOpt: "Create a poll (2–5 options)",
    addOption: "Add Option",
    multipleChoice: "Allow multiple choices",
    pollExpiration: "Poll expiration (optional)",

    //videos
    noVideo: "No video selected",
    selectVideo: "Select Video",
    notSeletThumbnail: "If the thumbnail is not selected, first frame will be used",
    seletThumbnail: "Select Thumnail",

    notiPage: {
      no_noti_yet: "အသိပေးစရာ အကြောင်းအရာ မရှိသေးပါ",
      upvote_noti: "က ပိုစ့်ကို Upvote လုပ်ထားပါတယ်",
      mention_noti: "က သင့်ကို ပိုစ့်ထဲမှာ Mention ခေါ်ထားပါတယ်",
      comment_noti: "က ပိုစ့်အောက်မှာ မှတ်ချက်ပေးထားပါတယ်",
      answer_noti: "မင်းရဲ့မေးခွန်းကို ငါဖြေတယ်။",
      follow_noti: "မင်းနောက်ကိုလိုက်ခဲ့တယ်။",
      system_noti: "Jearnအဖွဲ့အစည်းမှ အသိပေးချက်",
    },

    // comment
    comments: "မှတ်ချက်များ",
    addComment: "မှတ်ချက်ရေးမည်",
    writeComment: "မှတ်ချက်ဘယ်လိုပေးကြမလဲ?",
    editComment: "ပြုပြင်မည်",
    postComment: "မှတ်ချက်ပေးမည်",
    reply: "စာပြန်မည်",

    userMenu: {
      profile: "ပရိုဖိုင်",
      toggleTheme: "Themeပြောင်းမည်",
      requestCategory: "အမျိုးအစားအသစ်တောင်းဆိုရန်",
    },
    dashboard: "Dashboard",

    profilePage: {
      userNotFound: "အကောင့်ရှာမတွေ့ပါ",
      postsByBefore: "",
      postsByAfter: "၏ပို့စ်များ",
      following: "following",
    },

    profileSetting: {
      settings: "ပရိုဖိုင် ဆက်တင်များ",
      name: "နာမည်",
      bio: "မိတ်ဆက်စကား",
      uniqueId: "Unique ID",
      change: "Change",
      save: "Save changes",
      saving: "Saving...",
    },

    // dashboard
    overview: "ခြုံငုံသုံးသပ်ချက်",
    analytics: "ခွဲခြမ်းစိတ်ဖြာမှု",
    database: "ဒေတာဘေ့စ်",
    settings: "ပြုပြင်ခြင်း",
    notification: "အသိပေးချက်",
    reports: "တိုင်ကြားစာ",
    categoryRequest: "အမျိုးအစားတောင်းဆိုမှု",
    loadingDashboard: "Loading dashboard",
  },
};

export default my;
