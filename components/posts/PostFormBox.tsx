// components/posts/PostFormBox.tsx
"use client";
...
export default function PostFormBox({ open, onClose, onSubmit }: PostFormBoxProps) {
  return (
    <Portal>
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            aria-modal="true"
            role="dialog"
            // ✅ No onClick here — prevent outside close
          >
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()} // ✅ Blocks click inside modal content
            >
              {/* Header */}
              <header className="flex items-center justify-between p-4 border-b">
                <h2 className="text-lg font-semibold">Create Post</h2>
                <button onClick={onClose} className="text-xl hover:text-gray-600">✕</button>
              </header>

              {/* Content */}
              <section className="flex-1 overflow-y-auto p-6">
                <PostForm
                  onSubmit={async (title, content, authorId) => {
